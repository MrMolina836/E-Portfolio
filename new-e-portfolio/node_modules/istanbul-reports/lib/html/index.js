    ...
    function metricsTemplate({ pct, covered, total }, kind) {
        return `
            <div className='fl pad1y space-right2'>
                ...
                <span className="quiet">${kind}</span>
                <span className='fraction'>${covered}/${total}</span>
            </div>
        `;
    }
    ...
    function skipTemplate(metrics) {
        ...
        return `
            <div className='fl pad1y'>
                <span className="strong">${skips.join(', ')}</span>
...
<div className='wrapper'>
    ...
    <div className='status-line ${details.reportClass}'></div>
    `;
}

function footerTemplate(details) {
    return `
                <div className='push'></div><!-- for sticky footer -->
            </div><!-- /wrapper -->
...
function detailTemplate(data) {
    ...
    const lineCount = line =>
        `<span className="cline-any cline-${line.covered}">${line.hits}</span>`;

    /* This is rendered in a `<pre>`, need control of all whitespace. */
    return [
        '<tr>',
        `<td className="line-count quiet">${lineNumbers
            .map(lineLink)
            .join('\n')}</td>`,
        `<td className="line-coverage quiet">${data.lineCoverage
            .map(lineCount)
            .join('\n')}</td>`,
        `<td className="text"><pre className="prettyprint lang-js">${data.annotatedCode.join(
            '\n'
        )}</pre></td>`,
        '</tr>'
    ].join('');
...
function summaryLineTemplate(details) {
    ...
    const percentGraph = pct => {
        ...
        pct = Math.floor(pct);
        return [
            `<div className="${cls.join(' ')}" style="width: ${pct}%"></div>`,
            `<div className="cover-empty" style="width: ${100 - pct}%"></div>`
        ].join('');
    };
    const summaryType = (type, showGraph = false) => {
        ...
        const result = [
            `<td data-value="${info.pct}" className="pct ${reportClass}">${info.pct}%</td>`,
            `<td data-value="${info.total}" className="abs ${reportClass}">${info.covered}/${info.total}</td>`
        ];
        if (showGraph) {
            result.unshift(
                `<td data-value="${info.pct}" className="pic ${reportClass}">`,
                `<div className="chart">${percentGraph(info.pct)}</div>`,
                `</td>`
            );
        }
...
class HtmlReport extends ReportBase {
    ...
    onDetail(node, context) {
        ...
        const cw = this.getWriter(context).writeFile(linkMapper.getPath(node));
        cw.write(headerTemplate(templateData));
        cw.write('<pre><table className="coverage">\n');
